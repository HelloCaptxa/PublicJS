...
function _widget(el,opts,done,fail){
  ...
  function runComplex(){
    ...
    _get(base,'/challenge/complex').then(function(r){
      ...
      var px=pz.piece_start_x||0,py=pz.piece_start_y||pz.piece_y||0;
      ...
      cv.addEventListener('pointerdown',function(e){e.preventDefault();var c=gc(e);drag=true;dox=c.x-px;doy=c.y-py;traj=[[Math.round(c.x),Math.round(c.y),e.timeStamp]];cv.classList.add('drag');if(cv.setPointerCapture)cv.setPointerCapture(e.pointerId);});
      cv.addEventListener('pointermove',function(e){if(!drag)return;e.preventDefault();var evs=(e.getCoalescedEvents?e.getCoalescedEvents():null)||[e];for(var i=0;i<evs.length;i++){var c=gc(evs[i]),t=evs[i].timeStamp;if(traj.length>0){var l=traj[traj.length-1],dist=Math.hypot(c.x-l[0],c.y-l[1]);if(dist>5){var st=Math.ceil(dist/5);for(var s=1;s<st;s++){var rr=s/st;traj.push([Math.round(l[0]+(c.x-l[0])*rr),Math.round(l[1]+(c.y-l[1])*rr),l[2]+(t-l[2])*rr]);}}}px=Math.round(Math.max(0,Math.min(pW-pSz,c.x-dox)));py=Math.round(Math.max(0,Math.min(pH-pSz,c.y-doy)));traj.push([Math.round(c.x),Math.round(c.y),t]);}draw();chkRdy();});
      function onEnd(e){if(!drag)return;drag=false;cv.classList.remove('drag');if(cv.releasePointerCapture&&e.pointerId!=null)cv.releasePointerCapture(e.pointerId);placed=px>0;chkRdy();}
      ...
      btn.addEventListener('click',function(){
        ...
        var t0=traj.length?traj[0][2]:0;
        var rt=traj.map(function(p){return[p[0],p[1],p[2]-t0];});
