(function(W,D){'use strict';var _wSrc='const K=new Uint32Array([0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2]);function rotr(x,n){return(x>>>n)|(x<<(32-n))}function sha256(data){const msg=new Uint8Array(data);const len=msg.length;const bitLen=len*8;const padLen=((len+9)%64===0?len+9:len+9+(64-(len+9)%64));const padded=new Uint8Array(padLen);padded.set(msg);padded[len]=0x80;const dv=new DataView(padded.buffer);dv.setUint32(padLen-4,bitLen,false);let h0=0x6a09e667,h1=0xbb67ae85,h2=0x3c6ef372,h3=0xa54ff53a,h4=0x510e527f,h5=0x9b05688c,h6=0x1f83d9ab,h7=0x5be0cd19;const W=new Uint32Array(64);for(let i=0;i<padded.length;i+=64){for(let j=0;j<16;j++)W[j]=dv.getUint32(i+j*4,false);for(let j=16;j<64;j++){const s0=rotr(W[j-15],7)^rotr(W[j-15],18)^(W[j-15]>>>3);const s1=rotr(W[j-2],17)^rotr(W[j-2],19)^(W[j-2]>>>10);W[j]=(W[j-16]+s0+W[j-7]+s1)|0}let a=h0,b=h1,c=h2,d=h3,e=h4,f=h5,g=h6,h=h7;for(let j=0;j<64;j++){const S1=rotr(e,6)^rotr(e,11)^rotr(e,25);const ch=(e&f)^(~e&g);const t1=(h+S1+ch+K[j]+W[j])|0;const S0=rotr(a,2)^rotr(a,13)^rotr(a,22);const maj=(a&b)^(a&c)^(b&c);const t2=(S0+maj)|0;h=g;g=f;f=e;e=(d+t1)|0;d=c;c=b;b=a;a=(t1+t2)|0}h0=(h0+a)|0;h1=(h1+b)|0;h2=(h2+c)|0;h3=(h3+d)|0;h4=(h4+e)|0;h5=(h5+f)|0;h6=(h6+g)|0;h7=(h7+h)|0}const out=new Uint8Array(32);const ov=new DataView(out.buffer);[h0,h1,h2,h3,h4,h5,h6,h7].forEach((v,i)=>ov.setUint32(i*4,v,false));return out.buffer}const R_HEX=/^[0-9a-fA-F]+$/;self.onmessage=function(e){var d=e.data;var raw=d.challengeHex;var diff=d.difficulty;var buf=new Uint8Array(raw.length/2);for(var i=0;i<raw.length;i+=2)buf[i/2]=parseInt(raw.substr(i,2),16);var maxSteps=diff*10;for(var nonce=0;nonce<maxSteps;nonce++){buf[buf.length-4]=(nonce>>>24)&0xFF;buf[buf.length-3]=(nonce>>>16)&0xFF;buf[buf.length-2]=(nonce>>>8)&0xFF;buf[buf.length-1]=nonce&0xFF;var hash=sha256(buf.buffer);var valid=true;for(var j=0;j<diff;j++){if((new Uint8Array(hash))[j]!==0){valid=false;break}}if(valid){var hexNonce=nonce.toString(16);while(hexNonce.length<8)hexNonce='0'+hexNonce;self.postMessage({type:'done',nonce:hexNonce});return}if(nonce%Math.max(1,Math.floor(maxSteps/100))===0){var pct=Math.round((nonce/maxSteps)*90);self.postMessage({type:'progress',pct:pct})}}self.postMessage({type:'error',msg:'PoW exhausted'})}';
function _mkWorker(){var b=new Blob([_wSrc],{type:'application/javascript'});var url=URL.createObjectURL(b);var w=new Worker(url);w._blobUrl=url;return w;}
function _pow(hex,diff,onPct){return new Promise(function(ok,fail){var w=_mkWorker();var tm=setTimeout(function(){w.terminate();URL.revokeObjectURL(w._blobUrl);fail(new Error('PoW timed out'));},65000);w.onmessage=function(e){var d=e.data;if(d.type==='progress'){if(onPct)onPct(d.pct);}else if(d.type==='done'){clearTimeout(tm);w.terminate();URL.revokeObjectURL(w._blobUrl);ok(d.nonce);}else if(d.type==='error'){clearTimeout(tm);w.terminate();URL.revokeObjectURL(w._blobUrl);fail(new Error(d.msg));}};w.onerror=function(e){clearTimeout(tm);w.terminate();URL.revokeObjectURL(w._blobUrl);fail(e);};w.postMessage({challengeHex:hex,difficulty:diff});});}
...
function _noopE(e){try{if(console&&console.error)console.error('Captxa:',e);}catch(x){}}
...
(opts.mode==='complex')?runComplex():runSimple();
function runSimple(){
...
var powNonce=null,powDone=false,powOk=false;
_pow(hex,diff,sb).then(function(n){powNonce=n;powDone=true;powOk=true;chkRdy();bar.style.width='100%';}).catch(function(e){track.style.display='none';sm('PoW error','err');powDone=true;if(fail)fail(e);});
pzEl.style.display='block';
var pW=pz.width||400,pH=pz.height||300,pSz=pz.piece_size||pz.pieceSize||80;
var px=pz.piece_start_x||0,py=pz.piece_y||pz.piece_start_y||0,sx=px,sy=py;
...
bgI.onerror=pcI.onerror=function(){track.style.display='none';sm('Failed to load puzzle','err');if(fail)fail(ne('Puzzle image load error'));};
...
function chkRdy(){btn.disabled=!(powDone&&powOk&&placed);}
...
draw();placed=placed||(traj.length>1||px!==sx||py!==sy);chkRdy();
});
function onEnd(e){
...
placed=placed||(traj.length>1||px!==sx||py!==sy);chkRdy();
}
...
if(el._captxaRendered)return;
el._captxaRendered=true;
_widget(el,opts,function(token){_injectToken(opts,token);if(opts.onVerify)try{opts.onVerify(token);}catch(e){console.error('Captxa onVerify error:',e);}},opts.onError||_noopE);
}
};
})(window,document);
